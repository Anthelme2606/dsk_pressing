...
<head>
  ...
  <link rel="stylesheet" href="/public{{asset('/public/bower_components/material-scrolltop-master/src/material-scrolltop.css') }}">
...
<!-- Main content -->
    <section class="invoice">
      ...
      <div class="row">
        ...
        <div class="col-xs-3">
          <p><b>Avance donnée :</b> {{$deposit->advanced}}</p>
          <p><b>Remise :</b> {{ $deposit->discount }}</p>
        ...
        <div class="col-xs-12 table-responsive">
          <table class="table">
            <thead>
              <tr>
                  ...
                  <th><label>Qte total</label></th>
                  <th><label>Qte Livrée</label></th>
                  <th><label>Qte rest</label></th>
                  <th><label>P.U.</label></th>
                  <th><label>Montant</label></th>
                  <th><label>Qte à livrer</label></th>
                  <th><label>Somme</label></th>
              </tr>
            </thead>
          ...
          <tbody>
            ...
            <tr>
                ...
                <td>
                    {{ $depot->retrieve_quantity? $depot->retrieve_quantity : 0 }}
                </td>
                <td>
                    {{  $depot->retrieve_quantity? $depot->quantity - $depot->retrieve_quantity : $depot->quantity }}
                </td>
                ...
                <td class="price">
                    @php
                        ...
                        if($depot->type_action == 0){
                            $uno_price = $depot->article()->classic_price;
                        }
                        if($depot->type_action == 1){
                            $uno_price = $depot->article()->repass_price;
                        }
                        if($depot->type_action == 2){
                            $uno_price = $depot->article()->express_price;
                        }

                    @endphp

                    <input type="number" id="price[]" value="{{$uno_price}}" readonly>
                </td>
                ...
                <td>
                    <input type="number" name="" id="qty[]" class="form-control" min="0" max="{{  $depot->retrieve_quantity? $depot->quantity - $depot->retrieve_quantity : $depot->quantity }}"/>
                </td>

                {{-- <script>
                    var qte = document.getElementById("qty");
                    var price = document.getElementById("price");
                    var amount = document.getElementById("amount");
                    qte.addEventListener("input", function(){
                        amount.value = qte.value * price.value;
                    });
                </script> --}}
                <td class="amount">
                    <input type="text" value="0" id="amount[]" readonly>
                </td>
            ...
            <tr>
                ...
                <td></td>
                <td>
                    <p><b>Montant restant global :</b> {{ $deposit->total - $deposit->advanced - $deposit->discount }}</p>
                    <p><b>Total du retrait :</b> 0</p>
                    <p><b>Avance :</b> <input type="number" class="form-control" name="advanced"></p>
                </td>
            </tr>
      ...
      <div class="row no-print">
        <div class="col-xs-12">
          ...
          <a href="{{ route('ticket_casheer', $deposit->id) }}" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Format ticket de caisser</a>
          @role('manager')
