    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper" >
          ...
          <section class="content-header" >
              <h1>
                Dépôt
                <small>PE-{{ $deposit->agence() }} / {{ $deposit->id }}</small>
              </h1>
              <ol class="breadcrumb">
            ...
            <div class="row">
              <div style="text-align: center"><img src="{{ asset('public/website/images/z.png') }}" alt="Image du logo" width="40%" style="padding-top: -5%;" ></div>
                ...
                @if(env('IS_TEST') == true)
                    <h3><b>TEST</b></h3>
                  @endif
                <h4 class="text-center" style="font-style: italic;">{{ env('BY_WHO', 'By INNOV 720') }}</h4>
                <h5 class="text-center">{{ env('ADRESSE_LOCAL', 'PRESING ELEGANCE') }}</h5>
                <h5 class="text-center" style=" margin-bottom: 12px; font-style: italic;">{{ env('APP_RCCM', '') }}</h5>

                <h5 class="text-center" style="font-style: italic;"><b>PE-{{ $deposit->agence() }}/{{ $deposit->id }}</b> | <b>Client :</b>{{ $deposit->client()->fullname }}</h4>

                    <h6 class="text-center">Caissier(ère) : <b>{{ $deposit->user()->fullname }}</b></h6>
                <p style="text-align: center; font-size: 12px;">Date de réception :  {{$deposit->deposit_date->format("d/m/y")}}  | Date de retrait : <b> {{$deposit->retrieve_date->timezone('Europe/Brussels')->format("d/m/y h:i:s")}} </b><a href="javascript:;" data-toggle="modal" data-target="#editHour"><i class="fa fa-pencil no-print"></i></a> </p>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tfoot class="text-center">
                        ...
                        <tr>
                            @if($deposit->total - $deposit->advanced - $deposit->discount > 0)
                                <td style="font-size: 12px;"><b>Reste à payer:</b></td>
                                <td></td>

                                <td style="position: absolute; float:right; font-size: 12px;">{{ abs($deposit->total - $deposit->advanced - $deposit->discount ) }}</td>
                            @else
       ...
       <div class="row text-center" style="margin-top: -5px;">
        ...
        <div class="row text-center">
          ...
          @for($i=0; $i<$nbr_articles; $i++)
                <div>
                    <hr/>
                    <h5 class="text-center" style="font-style: italic; font-size: 30px;"><b>PE-{{ $deposit->agence() }}/{{ $deposit->id }}</b></h4>
                </div>
            @endfor
